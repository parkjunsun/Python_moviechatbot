...
from application import GOOGLE_API, MOVIE_API, CHAT_ID, LOTTE_TOKEN

welcome_msg = '반가워요.\n\n 저는 요즘 볼만한 영화들을 알려드리고, 현재 계신 곳에서 가까운 LotteCinema 영화관들의 상영시간표를 알려드려요.\n\n'\
              "'영화순위' , '근처 상영관 찾기', '예매하기', '길 찾기'를 입력해보세요"
error_msg = '잘 모르겠네요.\n\n'\
            '저는 요즘 볼만한 영화들을 알려드리고, '\
            '현재 계신 곳에서 가까운 영화관들의 상영시간표를 알려드려요.\n\n'\
            "'영화순위' , '근처 상영관 찾기' , '예매하기', '길 찾기'를 입력해보세요."
bot = telegram.Bot(token=LOTTE_TOKEN)
flag = 0
...
def send_nearest_theaters(chat_id,latitude,longitude):    
    ...
    bot.send_message(chat_id = chat_id, text = "현재위치를 확인했습니다.",reply_markup = reply_markup)
    cinema = LotteCinema()
...
def send_nearest_ticketbox(chat_id, latitude, longitude):
    ...
    d1 = nearest_tboxes[0]['TheaterID'].split('|')
    d2 = nearest_tboxes[1]['TheaterID'].split('|')
    d3 = nearest_tboxes[2]['TheaterID'].split('|')

    d1_divisionCode = d1[0]
    d1_detaildivisionCode = nearest_tboxes[0]['TheaterDCODE']
    d1_cinemaID = d1[2]

    d2_divisionCode = d2[0]
    d2_detaildivisionCode = nearest_tboxes[1]['TheaterDCODE']
    d2_cinemaID = d2[2]

    d3_divisionCode = d3[0]
    d3_detaildivisionCode = nearest_tboxes[2]['TheaterDCODE']
    d3_cinemaID = d3[2]

    url1 = 'https://www.lottecinema.co.kr/NLCMW/Cinema/Detail?divisionCode={}&detailDivisionCode={}&cinemaID={}'.format(d1_divisionCode, d1_detaildivisionCode, d1_cinemaID)
    url2 = 'https://www.lottecinema.co.kr/NLCMW/Cinema/Detail?divisionCode={}&detailDivisionCode={}&cinemaID={}'.format(d2_divisionCode, d2_detaildivisionCode, d2_cinemaID)
    url3 = 'https://www.lottecinema.co.kr/NLCMW/Cinema/Detail?divisionCode={}&detailDivisionCode={}&cinemaID={}'.format(d3_divisionCode, d3_detaildivisionCode, d3_cinemaID)
...
def build_url(method, query):
    return 'https://api.telegram.org/bot{}/{}?{}'.format(LOTTE_TOKEN,method,query)
